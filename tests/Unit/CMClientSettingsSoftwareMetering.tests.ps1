...
$script:dscResourceName = 'DSC_CMClientSettingsSoftwareMetering'
...
try
{
    InModuleScope $script:dscResourceName {
        ...
        Describe 'ConfigMgrCBDsc - CMClientSettingsSoftwareMetering\Get-TargetResource' -Tag 'Get' {
            BeforeAll {
                $clientReturn = @{
                    Type = 0
                }
                ...
                $smReturn = @{
                    Enabled                = $true
                    DataCollectionSchedule = 'DA159CC000280400'
                }
            ...
            Context 'When retrieving Client Policy Settings for software metering' {

                It 'Should return desired results when client settings exist' {
                    Mock -CommandName Get-CMClientSetting -MockWith { $clientReturn }
                    Mock -CommandName Get-CMClientSetting -MockWith { $smReturn } -ParameterFilter { $Setting -eq 'SoftwareMetering' }
                ...
                It 'Should return desired result when client setting policy exist but software metering settings is not configured' {
                    Mock -CommandName Get-CMClientSetting -MockWith { $clientReturn }
                    Mock -CommandName Get-CMClientSetting -MockWith { $null } -ParameterFilter { $Setting -eq 'SoftwareMetering' }
        ...
        Describe 'ConfigMgrCBDsc - CMClientSettingsSoftwareMetering\Set-TargetResource' -Tag 'Set' {
            BeforeAll {
                $returnPresentDevice = @{
                    SiteCode            = 'Lab'
                ...
                Mock -CommandName Set-CMClientSettingSoftwareMetering
                Mock -CommandName Import-ConfigMgrPowerShellModule
                Mock -CommandName Set-Location
            }
            ...
            Context 'When Set-TargetResource runs successfully' {
                ...
                It 'Should call expected commands when settings match' {
                    ...
                    Assert-MockCalled Set-CMClientSettingSoftwareMetering -Exactly -Times 0 -Scope It
                }
                ...
                It 'Should call expected commands when current state disabled and desired is enabled' {
                    ...
                    Assert-MockCalled Set-CMSchedule -Exactly -Times 0 -Scope It
                    Assert-MockCalled New-CMSchedule -Exactly -Times 0 -Scope It
                    Assert-MockCalled Set-CMClientSettingSoftwareMetering -Exactly -Times 1 -Scope It
                }
            ...
            Context 'When running Set-TargetResource should throw' {
                BeforeEach {
                    ...
                    $clientTypeError = 'Client Settings for software metering settings only applies to Default and Device client settings.'
        ...
        Describe 'ConfigMgrCBDsc - CMClientSettingsSoftwareMetering\Test-TargetResource' -Tag 'Test' {
            BeforeAll {
                $returnPresent = @{
                    SiteCode            = 'Lab'
                    ClientSettingName   = 'ClientTest'
